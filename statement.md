...
2. **Issue & Return Engine:** Check out and return books safely while updating persistent storage.
3. **Borrowing Limit Enforcement:** Restrict student checkout transactions if their current borrowing limit is reached.
4. **Fine Calculation:** Automatically evaluate daily penalty rates ($2.00/day) based on overdue duration.
5. **Robust Error Handling:** Throw and handle custom exceptions (`BookNotFoundException`, `UserLimitExceededException`) to ensure graceful application recovery.
